---
title: "Practical MachineLearning Project"
author: "Kanu Dutta"
...
```
As many observation in dataset contains NAs and blank . we first find the no of NAs and blanks in the dataset using apply, and then inspect the table which shows each row consisiting of a predictor and its number of NAs.Same procedure was adopted for blanks in the data.Finally remove the predictor consisiting of high number of NAs and blanks wrt to the total observations.

```{r,results='hide'}
# removing NAs and blanks
...
The split is 75% for training and 25% for validation
```{r}
...
```{r,eval=TRUE}
## centering and scaling for normalising the data
...
##### Visual inspection of the importance of variables, the map created using random forest 
```{r,fig.width=9, fig.height=10,results='hide',message=FALSE, }
...
##### training using knn , model was trained for 5 nearest neigbour 
```{r,eval=TRUE,message=FALSE}
...
##### combining all the four model based on majority voting.
The class appearing the most in a row was selected as the predicted class
```{r}
...
##### Random Forest insample vs out sample accuracy
...
#####  comparison between Random Forest and Ensemble
...
As random forest provides maximum accuracy in prediction across all classes, hence it was chosen to predict the classes in test cases.The out of sample accuracy for Randon Forest model is 
